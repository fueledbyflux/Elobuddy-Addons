...
namespace MrMundo.States
{
    class Combo : StateTemplate
    {
        ...
        {
            if (SpellHandler.E.IsReady() && Program.ComboMenu["useECombo"].Cast<CheckBox>().CurrentValue && EntityManager.Heroes.Enemies.Any(a => a.Name == target.Name && Player.Instance.IsInAutoAttackRange(a)))
            {
                SpellHandler.E.Cast();
            }
        }
